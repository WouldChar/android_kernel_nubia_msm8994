...
static int ak4375_amp_gpio=-1;
static int ak4375_handset_amp_gpio=-1;
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	ak4375->nBickFreq = 0;//ucontrol->value.enumerated.item[0];
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	regs = 0x00;
	rege = 0x15;
	printk("ak4375 set test reg i2c->addr:%x,i2c->name:%s",i2cdev->addr,i2cdev->name);
 ...
 */
static inline void ak4375_write_reg_cache(struct snd_soc_codec *codec, u16 reg,u16 value)
{
    u8 *cache = codec->reg_cache;
    BUG_ON(reg > ARRAY_SIZE(ak4375_reg));
    cache[reg] = (u8)value;
...
if(i2cdev->adapter==NULL)
	printk("ak4375_i2c_read i2cdev is null\n");
ret = i2c_smbus_read_byte_data(i2cdev, (u8)(reg & 0xFF));
...
//	if(i2c_smbus_write_byte_data(codec->control_data, (u8)(reg & 0xFF), (u8)(value & 0xFF))<0) {
		if(i2c_smbus_write_byte_data(i2cdev, (u8)(reg & 0xFF), (u8)(value & 0xFF))<0) {
...
static int ak4375_suspend(struct snd_soc_codec *codec)
{
	pr_err("[ZTEMT]%s\n",__func__);	
	ak4375_set_bias_level(codec, SND_SOC_BIAS_OFF);
	gpio_direction_output(ak4375_amp_gpio, 0);
	gpio_direction_output(ak4375_handset_amp_gpio, 0);
...
static int ak4375_resume(struct snd_soc_codec *codec)
{
	...
	gpio_direction_output(ak4375_handset_amp_gpio, 1);
	gpio_direction_output(ak4375_amp_gpio, 0);
	gpio_direction_output(ak4375_amp_gpio, 1);
	ak4375_init_reg(codec);
                            ...
                            const struct i2c_device_id *id)
{
	...
	ak4375_handset_amp_gpio = of_get_named_gpio(i2c->dev.of_node,
				"handset-gpio", 0);
	pr_err("[ZTEMT]%s  ak4375_handset_amp_gpio=%d\n",
	__func__,ak4375_handset_amp_gpio);	
	if (ak4375_handset_amp_gpio >= 0) gpio_request(ak4375_handset_amp_gpio, "handset-gpio");
	
	gpio_direction_output(ak4375_handset_amp_gpio, 1);
	

	ak4375_amp_gpio = of_get_named_gpio(i2c->dev.of_node,
				"en-gpio", 0);	
	pr_err("[ZTEMT]%s  ak4375_amp_gpio=%d\n",
		__func__,ak4375_amp_gpio);
	
	if (ak4375_amp_gpio >= 0) gpio_request(ak4375_amp_gpio, "en-gpio");
	...
	i2cdev = kzalloc(sizeof(struct i2c_client), GFP_KERNEL);
