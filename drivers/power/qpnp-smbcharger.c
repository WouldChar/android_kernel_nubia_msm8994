...
#define ZTE_EOC_PERIOD_FAST_MS	5000
#define ZTE_EOC_PERIOD_LOW_MS	10000
#undef  KERN_INFO
...
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
	struct delayed_work		zte_eoc_work;
	struct wake_lock		eoc_wake_lock;
	bool				chg_done_batt_full;
	bool				bad_charger;
	int				insert_charger_type;
#endif
};
...
out:
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
	{
		...
		if ((chip->chg_done_batt_full == true) &&
...
{
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
	DBG_CHARGE("Charger Contro:enable=%d \n",en);
	if (en && (get_prop_batt_status(chip) != POWER_SUPPLY_STATUS_CHARGING)) {
...
static void smbchg_aicl_deglitch_wa_en(struct smbchg_chip *chip, bool en)
{
	...
	if (chip->force_aicl_rerun)
		return;
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
	if (en && !chip->aicl_deglitch_short && !chip->bad_charger) {
#else
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (chip->bad_charger) {
		...
		if (rc < 0)
			dev_err(chip->dev, "could not read USB charger type property, rc=%d\n", rc);
...
#ifdef CONFIG_ZTEMT_SUPPORT_CHARGER_5V
...
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
		if (usb_present && bad_charger && !chip->bad_charger) {
			chip->bad_charger = bad_charger;
			rc = smbchg_sec_masked_write(chip,
				chip->misc_base + MISC_TRIM_OPT_15_8,
			...
			chip->usb_target_current_ma = min(chip->usb_target_current_ma,
				SMGCHARGE_IDEV_CHG_MAX);
			DBG_CHARGE("Adjust IUSB Current %d \n",
					chip->usb_target_current_ma);	
		}
 #endif
	}
...
static void eoc_work(struct work_struct *work)
{
	...
	struct smbchg_chip *chip = container_of(dwork, struct smbchg_chip, zte_eoc_work);
	static int count;
	int eoc_period_ms;
	bool usb_present;
	if(!chip)
...
#ifdef CONFIG_ZTEMT_MSM8994_CHARGER
	INIT_DELAYED_WORK(&chip->zte_eoc_work,  eoc_work);
	wake_lock_init(&chip->eoc_wake_lock, WAKE_LOCK_SUSPEND, "zte_eoc_lock");
#endif
