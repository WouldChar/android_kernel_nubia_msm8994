...
s32 gt1x_i2c_test(void)
{
	...
	while (retry++ < 2) {
		...
		if (!ret) {
			GTP_DEBUG("GTP_REG_HW_INFO : %08X", hw_info);
			return ret;
		}
...
s32 gt1x_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	...
	if (len > 16) {
		GTP_ERROR("i2c_read_dbl_check length %d is too long, do not beyond %lu", len, sizeof(buf));
		return ERROR;
	}
...
#endif
*/
		if((gt1x_wakeup_gesture && !gesture_enabled) || (!gt1x_wakeup_gesture)) {
